			...
			<script type="text/javascript">
				function getUrl(callback, filepath)
				{
					window.opener.CKEDITOR.tools.callFunction(callback, filepath);
			...
			<fieldset>
				...
				<div class="rmFolderPane left">
					...
					<ul class="rmFolderList">
						<?if($showUpOneLevel):?>
						<li><a href="/modal.php?class=ResourceManager&amp;mode=<?=$mode;?>&amp;currentDir=<?=$parentDir;?><?=$CKEargs;?>">^ Up One Level</a></li>
						<?endif;?>
						<?if($folders != null):?>
						<?foreach($folders as $key => $folder):?>
						<li><a href="/modal.php?class=ResourceManager&amp;mode=<?=$mode;?>&amp;currentDir=<?=$key;?><?=$CKEargs;?>"><?=$folder;?></a></li>
						<?endforeach;?>
						<?endif;?>
					</ul>
				...
				<div class="rmFilePane right">
					...
					<ul	class="rmFileList">					
						...
						<li><a href="#" onclick="getUrl('<?=$callback;?>', '<?=$filePath;?>')"><?=$fileName;?></a></li>
						<?endforeach;?>
					</ul>						
					<?else:?>
